...
## Process
If a new resource is found, **1) the link is added to the list** of [sources](#sources) below in the [unsorted](#unsorted) category. After **2) inspection** a resource it is **3) categorised**, moving it to the right/or a new category. After a more **4) thorough read** the link is accompanied with a **5) short summary** in brackets "[...]" behind the link in the sources list. If the content is irrelevant the link is **6a) removed and archived** from the sources list to the [archive](#archive) with a bracketed comment. This ends the process. If relevant **6b) further analysis** of the resource results in a easy to comprehend **7) excerpt**, added to the [content](#content) of this document, with a link referencing the original source. If not all content of a resource is used but the resource still relevant content for other parts of this explainer, this is **8a) noted** in the sources list next to the source in parenthesis "(...)". The link is **8b) removed and archived** from the sources list to the [archive](#archive) with a bracketed comment when all *relevant* content from the resource is added to the content of this document and the source is properly referenced with a hyperlink. (The very last link in the sources list below shows how to use markup on github).
