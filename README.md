...
## Process
If a new resource is found, **1) add the link** to the [sources](#sources) list below in the [unsorted](#unsorted) category. **2) inspect** the resource and **3) categorise** it, moving it to the correct or a new category. After **4) a more thorough read** the link is **5) summarized** in brackets "[...]" behind the link. If the content is irrelevant the link is **6a) removed and archived** from the sources list to the [archive](#archive) with a bracketed comment why. This ends the process. Otherwise **6b) further analysis** of the resource results in a easy to comprehend **7) excerpt**, added to the [content](#content) of this document. If not all content of a resource is used but the resource still contains relevant content for other parts of this explainer, this is **8a) noted** in the sources list next to the link in parenthesis "(...)". The link is **8b) removed and archived** with a bracketed comment when all *relevant* content from the resource is added to the content of this document and the source is properly referenced with a hyperlink. If a resource takes a lot of time, it is recommended to make a note of the status by commenting the process step and nickname behind the link as a bracketed comment to inform others.
